...
submitBtn.addEventListener('click', async (e) => {
  ...
  // key here.
  const {publishableKey} = await fetch('https://api-stripe-demo.rnqvist.com/checkout/config').then((r) => r.json());
  const stripe = Stripe(publishableKey, {
    apiVersion: '2020-08-27',
  });

  // Create the checkout session on the server.
  const {error, sessionId} = await fetch('https://api-stripe-demo.rnqvist.com/checkout/create-checkout-session', {
    method: 'POST',
    headers: {
